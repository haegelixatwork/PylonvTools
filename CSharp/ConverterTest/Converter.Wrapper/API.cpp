...
extern "C" CONVERTERWRAPPER_API const char** GetStringArray(int* num)
{
	vector<string> list;
	list.push_back("Test1");
	list.push_back("Test2");
	...
	*num = leng;
	...
	for (i = 0; i < leng; i++)
	{
		int len = MultiByteToWideChar(CP_ACP, 0, list[i].c_str(), -1, NULL, 0);
		wchar_t* wstr = new wchar_t[len + 1];
		memset(wstr, 0, len + 1);
		MultiByteToWideChar(CP_ACP, 0, list[i].c_str(), -1, wstr, len);
		len = WideCharToMultiByte(CP_UTF8, 0, wstr, -1, NULL, 0, NULL, NULL);
		char* str = new char[len + 1];
		memset(str, 0, len + 1);
		WideCharToMultiByte(CP_UTF8, 0, wstr, -1, str, len, NULL, NULL);
